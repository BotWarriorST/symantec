...
for i = 1 : 10
%i = 5;
filename = strcat('C:\Users\Philip\Documents\MATLAB\Data\CoffeeMachine', num2str(i));
filename = strcat(filename, '.txt');   
%filename = 'C:\Users\Philip\Documents\MATLAB\Data\Occupancy8.txt';
delimiter = '\t';
startRow = 2;
...
[motion_sig, win] = slidingWin(motion, 100, 5);
cntFromMot(i) = countEventNum(motion_sig);
%mot_final_sig = satCounter(motion_sig, 100);
[light_sig, win2] = slidingWin(abs(diff(light)), 100, 500);
light_final_sig = satCounter(light_sig, 50);
cntFromLight(i) = countEventNum(light_final_sig);

%{
figure;
subplot(4, 1, 1), plot(light);
title('original sensor data(A)');
subplot(4, 1, 2), plot(abs(diff(light)));
title('after diff and taking absolute value(C)');
subplot(4, 1, 3), plot(win2);
title('after sliding window(D)');
subplot(4, 1, 4), plot(light_final_sig);
